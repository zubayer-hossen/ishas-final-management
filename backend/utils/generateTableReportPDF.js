...
const generateTableReportPDF = ({ title, subtitle, columns, rows, orgName = 'ISHAS Organization' }) => {
  ...
  // 1. বাংলা ফন্ট রেজিস্টার করুন (আপনার ফন্ট ফাইলের সঠিক পাথ দিন)
  ...
  // -------- Header --------
  doc.fillColor('#4f46e5').fontSize(18).font('Bangla-Bold').text(orgName, { align: 'center' });
  doc.fillColor('#1f2937').fontSize(14).font('Bangla-Bold').text(title, { align: 'center' });
  
  if (subtitle) {
    doc.fillColor('#6b7280').fontSize(10).font('Bangla').text(subtitle, { align: 'center' });
  }
  doc.moveDown(1);
  ...
  // -------- Table Header --------
  doc.font('Bangla-Bold').fontSize(9).fillColor('#ffffff');
